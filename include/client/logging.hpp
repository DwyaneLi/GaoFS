...
#ifndef GAOFS_LOGGING_HPP
#define GAOFS_LOGGING_HPP
...
*/
    ...
    template <typename Buffer>
    static inline void
    format_syscall_info_to(Buffer&& buffer, gaofs::syscall::info info) {
        ...
        switch(gaofs::syscall::origin(info)) {
            case gaofs::syscall::from_internal_code:
                o = 'i';
                break;
            case gaofs::syscall::from_external_code:
                o = 'a';
                break;
            default:
                o = '?';
        ...
        switch(gaofs::syscall::target(info)) {
            case gaofs::syscall::to_hook:
                t = 'h';
                break;
            case gaofs::syscall::to_kernel:
                t = 'k';
                break;
            default:
                t = '?';
...
enum { max_buffer_size = LIBGAOFS_LOG_MESSAGE_SIZE };
...
static_buffer::grow(std::size_t size) {
    ...
    std::fprintf(
            stderr,
            "FATAL: message too long for gaofs::log::static_buffer, increase the size of\n"
            "LIBGAOFS_LOG_MESSAGE_SIZE in CMake or reduce the length of the offending "
            "message.\n");
    abort();
}
...
#else // !GAOFS_ENABLE_LOGGING

#define LOG_INFO(...)                                                          \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(gaofs::log::info, __func__,     \
...
#define LOG_WARNING(...)                                                       \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(gaofs::log::warning, __func__,  \
                                                __LINE__, __VA_ARGS__);        \
        }                                                                      \
    } while(0);
...
#define LOG_ERROR(...)                                                         \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(gaofs::log::error, __func__,    \
                                                __LINE__, __VA_ARGS__);        \
        }                                                                      \
    } while(0);
...
#define LOG_CRITICAL(...)                                                      \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(gaofs::log::critical, __func__, \
                                                __LINE__, __VA_ARGS__);        \
        }                                                                      \
    } while(0);
...
#define LOG_HERMES(...)                                                        \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(gaofs::log::hermes, __func__,   \
                                                __LINE__, __VA_ARGS__);        \
        }                                                                      \
    } while(0);
...
#define LOG_MERCURY(...)                                                       \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(gaofs::log::mercury, __func__,  \
                                                __LINE__, __VA_ARGS__);        \
        }                                                                      \
    } while(0);
...
#define LOG_TRACE_READS(...)                                                   \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(                               \
                    gaofs::log::trace_reads, __func__, __LINE__, __VA_ARGS__);  \
        }                                                                      \
    } while(0);
...
#define LOG_SYSCALL(...)                                                       \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log_syscall(__VA_ARGS__);          \
        }                                                                      \
    } while(0);
...
#define LOG_DEBUG(...)                                                         \
    do {                                                                       \
        if(gaofs::log::get_global_logger()) {                                   \
            gaofs::log::get_global_logger()->log(gaofs::log::debug, __func__,    \
                                                __LINE__, __VA_ARGS__);        \
        }                                                                      \
    } while(0);

#else // ! GAOFS_DEBUG_BUILD
...
#endif // ! GAOFS_DEBUG_BUILD
#endif // !GAOFS_ENABLE_LOGGING

#endif //GAOFS_LOGGING_HPP
