 ...
 */
    log_level
    process_log_options(const std::string gaofs_debug) {
        ...
        auto tokens = ::split_str(gaofs_debug, " ,", ::split_str_mode::is_any_of);
...
process_log_filter(const std::string& log_filter) {
    ...
    debug_verbosity_ = verbosity;
        ...
        if(!path.empty()) {
            ...
            if(fd == -1) {
                log(gaofs::log::error, __func__, __LINE__,
                    "Failed to open log "
