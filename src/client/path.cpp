...
unsigned int match_components(const string& path, unsigned int& path_components,
    ...
    string::size_type comp_size = 0; // 当前部分的size
    string::size_type start = 0; // 当前部分的起点
    string::size_type end = 0; // 当前部分的终点
...
// 将path的规范形式填充到resolved里，如果是gaofs里的返回true
...
// 将环境变量LIBGAOFS_CWD设置为path
void set_env_cwd(const string& path) {

    LOG(DEBUG, "Setting {} to \"{}\"", gaofs::env::CWD, path);

    if(setenv(gaofs::env::CWD, path.c_str(), 1)) {
        LOG(ERROR, "Failed while setting {}: {}", gaofs::env::CWD,
            ::strerror(errno));
        throw ::system_error(
...
void unset_env_cwd() {
    ...
    LOG(DEBUG, "Clearing {}()", gaofs::env::CWD);

    if(unsetenv(gaofs::env::CWD)) {

        LOG(ERROR, "Failed to clear {}: {}", gaofs::env::CWD, ::strerror(errno));
...
void init_cwd() {
    const char* env_cwd = ::getenv(gaofs::env::CWD);
    if(env_cwd != nullptr) {
        CTX->cwd(env_cwd);
    } else {
        CTX->cwd(get_sys_cwd());
