...
void Stats::setup_Prometheus(const std::string& gateway_ip,
...
void Stats::add_read(const std::string& path, unsigned long long chunk) {
    chunk_reads[pair(path, chunk)]++;
}

void Stats::add_write(const std::string& path, unsigned long long chunk) {
    chunk_writes[pair(path, chunk)]++;
}


void Stats::output_map(std::ofstream& output) {
    // Ordering
...
void Stats::add_value_iops(enum IopsOp iop) {
    iops_mean[iop]++;
    ...
    } else if(time_iops[iop].size() >= gaofs::config::stats::max_stats)
        time_iops[iop].pop_front();
...
void Stats::add_value_size(enum SizeOp iop, unsigned long long value) {
    auto now = std::chrono::steady_clock::now();
    size_mean[iop] += value;
    ...
    } else if(time_size[iop].size() >= gaofs::config::stats::max_stats)
        time_size[iop].pop_front();
 ...
 * @return mean value
 */
double Stats::get_mean(enum SizeOp sop) {
    auto now = std::chrono::steady_clock::now();
    auto duration =
...
double Stats::get_mean(enum IopsOp iop) {
    auto now = std::chrono::steady_clock::now();
    auto duration =
            std::chrono::duration_cast<std::chrono::seconds>(now - start);
...
std::vector<double> Stats::get_four_means(enum SizeOp sop) {
...
std::vector<double> Stats::get_four_means(enum IopsOp iop) {
    std::vector<double> results = {0, 0, 0, 0};
    auto now = std::chrono::steady_clock::now();
    const std::lock_guard<std::mutex> lock(time_iops_mutex);
...
void Stats::dump(std::ofstream& of) {
    for(auto e : all_IopsOp) {
...
void Stats::output(std::chrono::seconds d, std::string file_output) {
    int times = 0;
